...
module.exports = (
    ...
    let codeExampleDirectory = normalizePath('code-examples')

    visit(markdownAST, `inlineCode`, node => {
        const { value } = node

        const relativePathPrefix = getNode(markdownNode.parent).dir

        if (value.startsWith(`embed:`)) {
            const file = value.substr(6).trim()
            let path = normalizePath(`${codeExampleDirectory}/${file}`)
            const relativePath = `${relativePathPrefix}/${file}`

            if (!fs.existsSync(path)) {
                if (!fs.existsSync(relativePath)) {
                    throw Error(`Invalid snippet specified; no such file "${path}" or ${relativePath}`)
                }
                path = relativePath
            }
