...
1. Allow `authData` object on every request via the [http connection endpoint](../server/configuration#http) in order to send the login credentials
...
2. If your chosen auth strategy is webhook, a token can be sent in order to authenticate requests against the `endpointUrl`
...
3. If you're using open auth, you can just go ahead and make requests without authentication data.
...
|---|---|---|---|
|token|string|true|A valid auth token. Leave this out to use open auth.|
|authData|object|true|Authentication credentials. Leave this out to use open auth.|
...
For an update to succeed, the version provided must be 1 greater than the current version (see [head](#record-head)), otherwise a version conflict will occur.
