...
:::note
You can omit the `hash` key in order to use cleartext passwords (Not advised in production settings!). Otherwise, the provided password upon login must be the already hashed password using the same algorithm you specified in options.
:::
...
Start the deepstream server and you should see the authentication type confirmed in the startup log.
