...
deepstream can use a number of strategies to authenticate incoming connections. For JWT we'll use the [HTTP-Webhook](/docs/tutorials/core/auth/http-webhook/) - a configurable URL that deepstream will send both login and connection data to for verification.
...
![HTTP authentication flow](/img/tutorials/20-core/webhook-flow.png)
